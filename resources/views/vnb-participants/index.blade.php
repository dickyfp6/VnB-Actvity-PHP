		...
		<div class="overflow-x-auto">
			<table class="table-modern" id="participants-table" style="width: max-content; table-layout: auto;">
				...
				<tbody id="participants-body">
					<tr><td colspan="14" class="text-center py-8 text-gray-400">Belum ada participants.</td></tr>
				</tbody>
			</table>
		</div>
	</div>
...
async function loadParticipants() {
	...
	try {
		...
		if (managerIdFilter) params.set('manager_id', managerIdFilter);
		if (managerNameFilter) params.set('manager_name', managerNameFilter);

		const response = await fetch(`/api/vnb/participants?${params.toString()}`, {
			credentials: 'same-origin',
			headers: { 'Accept': 'application/json' },
		});
		const data = await response.json();
		participantsRows = Array.isArray(data?.data) ? data.data : [];
		// Ensure manager directory is loaded so we can derive manager names
		// when API response omits manager labels.
		await loadManagerEmployeeDirectory();
	...
	tbody.innerHTML = rows.map((row, index) => {
		...
		return `
			<tr>
				...
				<td>
					${row.employee_id
						? `<a href="/employees/${encodeURIComponent(row.employee_id)}" class="participants-manager-link hover:underline">${escapeHtml(row.name || '-')}</a>`
						: escapeHtml(row.name || '-')}
				</td>
