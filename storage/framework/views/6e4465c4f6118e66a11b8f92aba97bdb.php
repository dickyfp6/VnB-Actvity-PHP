    ...
    <div class="bg-white rounded-xl shadow-sm p-4 mb-4 md:mb-0">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-6 border-b border-gray-200">
                <button id="btn-lifecycle-active" onclick="setLifecycleTab('active')" class="inline-flex items-center border-b-2 border-green-800 px-1 py-2 font-medium text-[#144600] transition-colors rounded-none">Employee Active</button>
                <button id="btn-lifecycle-history" onclick="setLifecycleTab('history')" class="inline-flex items-center border-b-2 border-transparent px-1 py-2 font-medium text-gray-500 transition-colors hover:border-gray-300 hover:text-gray-700 rounded-none">History Employee</button>
            </div>
            
            <div class="flex items-center gap-2">
                <div class="relative">
                    <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="text" id="global-search" placeholder="Cari data..." class="pl-9 pr-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none w-full md:w-64" onkeyup="handleGlobalSearch()">
                </div>
                <button onclick="resetGlobalFilters()" class="px-3 py-2 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg border border-gray-200 transition whitespace-nowrap">
                    <i class="fas fa-undo-alt text-gray-500 mr-1"></i> Reset Filter
                </button>
            </div>
        </div>
    </div>
            ...
            <table class="table-modern" style="width: max-content; table-layout: auto;">
                <thead style="white-space: nowrap;">
                    <tr>
                        ...
                        <th class="text-center" data-column-key="employee_number">NIP</th>
                        <th class="text-center" data-column-key="name_display">Nama Lengkap</th>
                        <th class="text-center" data-column-key="date_joined">Tanggal Masuk</th>
                        <th class="text-center" data-column-key="email">Email</th>
                        <th class="text-center" data-column-key="whatsapp">Whatsapp</th>
                        <th class="text-center" data-column-key="manager_functional">Manager Fungsional</th>
                        <th class="text-center" data-column-key="manager_operational">Manager Operasional</th>
                        <th class="text-center" data-column-key="company">Perusahaan</th>
                        <th class="text-center" data-column-key="division">Divisi</th>
                        <th class="text-center" data-column-key="department">Departemen</th>
                        <th class="text-center" data-column-key="position">Jabatan</th>
                        <th class="text-center" data-column-key="placement">Penempatan</th>
                        <th class="text-center" data-column-key="level">Golongan</th>
                        <th class="text-center" data-column-key="employee_status">Status Pegawai</th>
                    </tr>
                </thead>
...
const managerIdFilter = urlParams.get('manager_id');
const managerNameFilter = urlParams.get('manager_name');
...
function updateLifecycleButtons() {
    ...
    if (currentLifecycle === 'active') {
        activeBtn.classList.add('border-green-800', 'text-[#144600]');
        activeBtn.classList.remove('border-transparent', 'text-gray-500');
        historyBtn.classList.add('border-transparent', 'text-gray-500');
        historyBtn.classList.remove('border-green-800', 'text-[#144600]');
    } else {
        historyBtn.classList.add('border-green-800', 'text-[#144600]');
        historyBtn.classList.remove('border-transparent', 'text-gray-500');
        activeBtn.classList.add('border-transparent', 'text-gray-500');
        activeBtn.classList.remove('border-green-800', 'text-[#144600]');
    }
}
...
async function loadEmployees() {
    document.getElementById('employee-body').innerHTML = '<tr><td colspan="15" class="text-center py-8 text-gray-400">Memuat data...</td></tr>';
    const params = new URLSearchParams({ lifecycle: currentLifecycle });
    if (managerIdFilter) {
        params.set('manager_id', managerIdFilter);
    }
    if (managerNameFilter) {
        params.set('manager_name', managerNameFilter);
    }

    const res = await apiGet(`/api/employees?${params.toString()}`);
...
function handleGlobalSearch() {
    globalSearchQuery = document.getElementById('global-search').value;
    renderTable();
}
...
function resetGlobalFilters() {
    ...
    if (table && window.resetTableEnhancer) {
        window.resetTableEnhancer(table);
    }
...
function renderTable() {
    ...
    if (!allEmployees.length) {
        ...
        tbody.innerHTML = `<tr><td colspan="15" class="text-center py-4 text-gray-400">${emptyText}</td></tr>`;
        return;
    }

    let filteredData = allEmployees;
    if (globalSearchQuery) {
        const searchTerms = globalSearchQuery.toLowerCase().trim().split(/\s+/);
        filteredData = allEmployees.filter(row => {
            const rowContent = [
                row.employee_number,
                ...
                row.email,
                row.whatsapp,
                row.manager_functional,
                row.manager_operational,
                row.company,
                row.division,
                row.department,
                row.position,
    ...
    if (!filteredData.length) {
        tbody.innerHTML = `<tr><td colspan="15" class="text-center py-4 text-gray-400">Tidak ada data yang cocok dengan pencarian "${escapeHtml(globalSearchQuery)}"</td></tr>`;
        return;
    }

    tbody.innerHTML = filteredData.map((row, idx) => {
        const detailQuery = managerIdFilter ? `?manager_id=${encodeURIComponent(managerIdFilter)}` : '';

        return `
        <tr class="hover:bg-gray-50">
            <td class="px-3 py-2 text-left" data-column-key="row_index">${idx + 1}</td>
            <td class="px-3 py-2 text-left" data-column-key="employee_number">${row.employee_number ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="name_display">
                <a
                    href="/employees/${row.id}${detailQuery}"
            ...
            <td class="px-3 py-2 text-left" data-column-key="date_joined">${row.date_joined ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="email">${row.email ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="whatsapp">${row.whatsapp ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="manager_functional">${row.manager_functional ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="manager_operational">${row.manager_operational ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="company">${row.company ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="division">${row.division ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="department">${row.department ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="position">${row.position ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="placement">${row.placement ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="level">${row.level ?? '-'}</td>
            <td class="px-3 py-2 text-left" data-column-key="employee_status">${row.employee_status ?? '-'}</td>
        </tr>
    `;
    }).join('');
}
