...
function buildManagerSelectOptions(employee, managerType, selectedId) {
	const suggested = managerType === 'functional' ? getSuggestedFunctionalManagers(employee) : [];
	...
	if (suggested.length > 0) {
		html += '<optgroup label="Suggested Managers">';
		suggested.forEach(m => {
			html += `<option value="${m.id}" ${String(m.id) === String(selectedId) ? 'selected' : ''}>${escapeHtml(m.name)}${m.division ? ' • ' + m.division : ''}</option>`;
		});
		html += '</optgroup>';
