...
async function showNewHires(id) {
  ...
  document.getElementById('hires-body').innerHTML = data.map(n => {
    ...
    <tr>
      ...
      <td class="px-4 py-2">${n.manager_operational || '-'}</td>
      <td class="px-4 py-2">${n.career_stage || '-'}</td>
      <td class="px-4 py-2">${n.phase || '-'}</td>
      <td class="px-4 py-2 font-medium">${n.progress || 0}%</td>
    </tr>`;
  }).join('');
}
